...
int main(int argc, char* argv[]) {
  ...
  QSettings nutucoinKey("HKEY_CLASSES_ROOT\\nutucoin", QSettings::NativeFormat);
  nutucoinKey.setValue(".", "Nutucoin Wallet");
  nutucoinKey.setValue("URL Protocol", "");
  QSettings nutucoinOpenKey("HKEY_CLASSES_ROOT\\nutucoin\\shell\\open\\command", QSettings::NativeFormat);
  nutucoinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = Nutucoin URL Handler\\nGenericName = Nutucoin\\nComment = Handle URL Sheme nutucoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/nutucoin;\\nIcon = Nutucoin-Wallet' | tee /usr/share/applications/nutucoin-handler.desktop";

...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("nutucoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "nutucoin:");
    }
    if(!_request.startsWith("nutucoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with nutucoin:"), QtCriticalMsg));
      return;
    }
